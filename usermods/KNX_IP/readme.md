...
It runs over the active WLED network interface and supports both Wi-Fi and Ethernet on ESP32 builds. AP-only mode without an upstream network link does not provide a usable KNX/IP service.
...
For Ethernet-only deployments, use one of the example environments that enables `WLED_USE_ETHERNET` in `platformio_override_KNX_IP.ini`.
...
## Todo
- In Wi-Fi Access Point mode without a real Wi-Fi or Ethernet uplink, it will not run the real KNX_IP service.
